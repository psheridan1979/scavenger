...
for i = 1:length(tickers)
    ...
    if status
        ...
        stocks(idx).Open = single(str2double(op));      % save opening price data
        stocks(idx).High = single(str2double(high));    % save high price data
        stocks(idx).Low = single(str2double(low));      % save low price data
        stocks(idx).Close = single(str2double(cl));     % save closing price data
        stocks(idx).Volume = single(str2double(volume));      % save volume data
        stocks(idx).AdjClose = single(str2double(adj_close)); % save adjustied close data
        %stocks(idx).Calcs = struct();
        stocks(idx).Description = ' ';  %added to hold description info
